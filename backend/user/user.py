    ...
    try:
        if routing_key == "tick":
            for recipient in connections.keys():
                if (
                    "company" in connections[recipient]
                    and "socket" in connections[recipient]
                ):
                    company_data = db_retrieve(
                        """SELECT * FROM "COMPANIES" 
                                                        WHERE "NAME"=%s""",
                        (connections[recipient]["company"],),
                    )
                    company_data = [
                        {
                            "timestamp": body.decode("ascii"),
                            "name": row[0],
                            "cash": row[1],
                            "features": row[2],
                            "valuation": row[3],
                    ...
                    try:
                        await connections[recipient]["socket"].send(
            ...
            for recipient in connections.keys():
                ...
                if "company" in connections[recipient]:
                    output_data = db_retrieve(
                        """SELECT "EMPLOYEES"."NAME", "EMPLOYER", "PRIORITY", "SKILL", "SALARY"
            ...
            if recipient.startswith("company"):
                ...
                for recipient in connections.keys():
                    if (
                        "company" in connections[recipient]
                        and "socket" in connections[recipient]
                        and connections[recipient]["company"] == company
                    ):
...
async def handle_user_input(websocket, path):
    ...
    try:
        async for message in websocket:
            ...
            if args["message"] == "heartbeat":
                ...
                cursor.execute(
                    ...
                    VALUES (%s, 0, 0, 0, 0)
                     ON CONFLICT ("NAME")
                     DO UPDATE SET "CASH"=0, "FEATURES"=0, "VALUATION"=0, "ARR"=0""",
                    (company,),
                )
        ...
        connections = {
            ...
            or ("socket" in value and value["socket"] != websocket)
        }
